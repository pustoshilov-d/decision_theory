    ...
    def eval(self):
        ...
        while not Ostanov:
            ...
            if np.linalg.det(self.Gesse) > 0:
                self.GesseInv = np.linalg.inv(self.Gesse)
            else:
                self.GesseInv = np.eye(self.n)
            ...
            self.X += - self.h * np.dot(self.GesseInv, self.grad)
...
if __name__ == '__main__':
    ...
    my_NutonRafson.eval()
    print('res: ', my_NutonRafson.result, f(my_NutonRafson.result))